...
function actualizarMenu() {
  const usuarios = fs.existsSync(USUARIOS_DIR)
    ? fs.readdirSync(USUARIOS_DIR).filter(n =>
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    { label: `Estado: ${estado.backendActivo ? '🟢 Backend' : '🔴 Backend'}, ${estado.tunnelActivo ? '🟢 Tunnel' : '🔴 Tunnel'}`, enabled: false },
    { type: 'separator' },
    { label: '⚙ Encender servidor', click: iniciarServidor },
    { label: '🔁 Reiniciar servidor', click: () => { detenerServidor(); iniciarServidor(); } },
    { label: '🛑 Apagar servidor', click: detenerServidor },
    { type: 'separator' },
    { label: '🙋 Elegir usuario', submenu: usuarioMenu },
    { type: 'separator' },
    { label: '❌ Cerrar programa', click: () => { detenerServidor(); app.quit(); } }
  ]);

  tray.setContextMenu(contextMenu);
  tray.setToolTip(`GPT sin Alzheimer - ${estado.usuario || 'Sin usuario activo'}`);
}
...
app.on('window-all-closed', (e) => {
  ...
  ipcMain.on('usuarioNuevo', (event, nombre) => {
    ...
    if (nombreLimpio === '' || nombreProhibido.test(nombreLimpio)) {
      dialog.showErrorBox('Nombre inválido', 'Ese nombre no es válido en Windows.');
      return;
    }
  
    const ruta = path.join(USUARIOS_DIR, nombreLimpio);
    if (fs.existsSync(ruta)) {
      dialog.showErrorBox('Usuario ya existe', `El usuario "${nombreLimpio}" ya está registrado.`);
      return;
    }
